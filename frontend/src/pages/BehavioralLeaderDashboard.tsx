...
export default function BehavioralLeaderDashboard() {
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-10 border-b border-border bg-white px-6 py-4">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm text-muted-foreground">Líder Comportamental</p>
            <h1 className="text-2xl font-semibold text-slate-950">Dashboard do Líder Comportamental</h1>
          </div>
          <button
            onClick={() => navigate('/')}
          ...
          <div className="mt-6 space-y-4">
            {selectedProject.employees.map((employee) => (
              <div key={employee.id} className="flex flex-col gap-4 rounded-3xl border border-border bg-slate-50 p-4 md:flex-row md:items-center md:justify-between">
                ...
                <button
                  onClick={() => navigate(`/behavioral-leader/evaluate/${selectedProject.id}/${employee.id}`)}
