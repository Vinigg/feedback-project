...
import { User, TrendingUp, Award, AlertCircle, ArrowLeft, History, ThumbsUp, AlertTriangle } from "lucide-react";
...
interface EmployeeData {
  ...
  email: string;
  averageScore: number;
  performanceData: { id: string; month: string; score: number }[];
  skillsData: { id: string; skill: string; value: number }[];
  projects: { id: string; name: string; role: string; status: string }[];
  recentFeedbacks: {
    id: number;
    project: string;
    date: string;
...
const mockEmployees: Record<string, EmployeeData> = {
  ...
  'emp-2': {
    ...
    averageScore: 3.9,
    performanceData: [
      { id: 'perf-1', month: 'Jan', score: 3.6 },
      { id: 'perf-2', month: 'Fev', score: 3.8 },
      { id: 'perf-3', month: 'Mar', score: 4.0 },
      { id: 'perf-4', month: 'Abr', score: 4.1 },
    ],
    skillsData: [
      { id: 'skill-1', skill: 'Velocidade', value: 4.0 },
      { id: 'skill-2', skill: 'Qualidade', value: 4.2 },
      { id: 'skill-3', skill: 'Iniciativa', value: 3.8 },
      { id: 'skill-4', skill: 'Comunicação', value: 3.7 },
      { id: 'skill-5', skill: 'Proatividade', value: 3.9 },
    ],
    projects: [
      { id: 'proj-1', name: 'API Gateway', role: 'Backend Developer', status: 'active' },
      { id: 'proj-2', name: 'Microsserviços', role: 'Tech Lead', status: 'active' },
    ],
    recentFeedbacks: [
      {
        id: 1,
        ...
        technicalStrengths: ['Arquitetura', 'Performance', 'Segurança'],
        behavioralStrengths: ['Liderança', 'Autonomia'],
        technicalImprovements: ['Documentação'],
        behavioralImprovements: [],
        technicalHighlight: 'Excelente domínio técnico em APIs REST',
        behavioralHighlight: 'Referência positiva para o time',
      },
    ],
  },
};
...
export default function AdminEmployeeProfile() {
  ...
  const employee = id ? mockEmployees[id] : null;
      ...
      <header className="bg-[#1a1a1a] border-b border-border sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg sm:text-xl text-white">{employee.name}</h1>
              <p className="text-xs sm:text-sm text-white/60">{employee.role}</p>
            </div>
          </div>
          <button
            onClick={() => navigate('/admin/employees')}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700">
              Você está visualizando o perfil de <strong>{employee.name}</strong> como administrador.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 sm:mb-8">
          <div className="bg-white rounded-xl p-6 border border-border">
            ...
            <p className="text-3xl mb-1">{employee.averageScore.toFixed(1)}</p>
          ...
          <div className="bg-white rounded-xl p-6 border border-border">
            ...
            <p className="text-3xl mb-1">{employee.recentFeedbacks.length}</p>
          ...
          <div className="bg-white rounded-xl p-6 border border-border">
            ...
            <p className="text-3xl mb-1">{employee.projects.filter(p => p.status === 'active').length}</p>
          ...
          <div className="bg-white rounded-xl p-6 border border-border">
            ...
            <p className="text-sm text-muted-foreground">
              Score: {Math.max(...employee.skillsData.map(s => s.value)).toFixed(1)}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 border border-border">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={employee.performanceData}>
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 border border-border">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <RadarChart data={employee.skillsData}>
        ...
        <div className="bg-white rounded-xl border border-border">
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-border flex items-center gap-2">
            ...
            <h3 className="text-base sm:text-lg">Feedbacks Recentes</h3>
          </div>
          <div className="p-4 sm:p-6">
            <div className="space-y-4 sm:space-y-6">
              {employee.recentFeedbacks.map((feedback, idx) => (
                <div key={feedback.id} className="relative pl-6 sm:pl-8">
                  {idx !== employee.recentFeedbacks.length - 1 && (
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        ...
                        <div className="text-xs sm:text-sm text-muted-foreground mt-1 space-y-1">
                          {feedback.technicalLeader && <p>📘 Técnico: {feedback.technicalLeader}</p>}
                          {feedback.behavioralLeader && <p>💜 Comportamental: {feedback.behavioralLeader}</p>}
                        </div>
                      </div>
                      ...
                      <div className="mb-3 pb-3 border-b border-border">
                        ...
                        {feedback.technicalStrengths && feedback.technicalStrengths.length > 0 && (
                          <div className="mb-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {feedback.technicalStrengths.map((s, i) => (
                                <span key={i} className="px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded text-xs border border-emerald-200">{s}</span>
                              ))}
                            </div>
                          </div>
                        )}
                        {feedback.technicalImprovements && feedback.technicalImprovements.length > 0 && (
                          <div className="mb-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {feedback.technicalImprovements.map((s, i) => (
                                <span key={i} className="px-2 py-0.5 bg-amber-50 text-amber-700 rounded text-xs border border-amber-200">{s}</span>
                              ))}
                            </div>
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        {feedback.behavioralStrengths && feedback.behavioralStrengths.length > 0 && (
                          <div className="mb-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {feedback.behavioralStrengths.map((s, i) => (
                                <span key={i} className="px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded text-xs border border-emerald-200">{s}</span>
                              ))}
                            </div>
                          </div>
                        ...
                        {feedback.behavioralImprovements && feedback.behavioralImprovements.length > 0 && (
                          <div className="mb-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {feedback.behavioralImprovements.map((s, i) => (
                                <span key={i} className="px-2 py-0.5 bg-amber-50 text-amber-700 rounded text-xs border border-amber-200">{s}</span>
                              ))}
                            </div>
                          </div>
